...
| [degregorio-blowup](https://github.com/munawarkazmi/degregorio-blowup) | Finite-time blowup in the Okamoto, Sakajo and Wunsch family of 1D models for 3D Euler, on the circle - a deliberate way into the Navier-Stokes problem through something one person can hold in view. The frozen blowup profile and the regime diagram around it are Lushnikov, Silantyev and Siegel's, and the paper says so; what this adds is the local theory for that profile. It can only be singular where its own velocity vanishes, there are exactly two such stagnation points, and they sit exactly pi apart - forced by a parity argument, not observed. The local exponent there predicts the profile's entire spectral decay from a single number rather than from a least-squares fit to the tail. Using the stagnation point that has a closed form as a control variate for the one that does not pins that exponent to five digits: 3.0024227 at a = 0.8, which excludes the natural guess of exactly 3 by 500x the scatter, and 9.29546 at a = 0.71 against the published fitted 9.3259. The blowup time from sin x at a = 1/2 is exactly pi, checked against the solver at 1e-14. 19 validation checks, every result tested against a closed form before it is believed - and four constants retracted along the way when they failed that test. Floating-point numerics with convergence studies, not proof, and the paper states plainly which results are prior work. |
